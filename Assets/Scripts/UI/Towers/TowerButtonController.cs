...
public class TowerButtonController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < _towerPurchaseButtons.Count; i ++)
            _towerPurchaseButtons[i].Initialize(buffer[i].Icon, buffer[i].Price);
    }
    ...
    private void Update()
    {
        if (_towerPurchaseButtons[0].gameObject.activeSelf)
        {
            for (int i = 0; i < _towerPurchaseButtons.Count; i++)
            {
                TowerData buffer = _playerController.Towers[i];

                _towerPurchaseButtons[i].UpdateState(buffer.Price <= _ressourceController.GoldCount);
            }
        }
        else if (_towerUpgradesButtons[0].gameObject.activeSelf)
        {
            for (int i = 0; i < _towerUpgradesButtons.Count; i++)
            {
                TowerData buffer = _playerController.Towers[i];

                _towerUpgradesButtons[i].UpdateState(buffer.Price <= _ressourceController.GoldCount);
            }
        }
    }
    ...
    public void ActivateTowerUpgradeButtons(Vector2 newPosition, Tower newTower, int newPrice)
    {
        ...
        if(_currentTower.Data.Upgrades.Count > 1)
        {
            for (int i = 0; i < _towerUpgradesButtons.Count; i++)
            {
                if (i == 0)
                {
                    _towerUpgradesButtons[0].gameObject.SetActive(true);
                    _towerUpgradesButtons[0].Initialize(_currentTower.Data.Upgrades[0].Icon, _currentTower.Data.Upgrades[0].Price);

                    _towerUpgradesButtons[0].ChangeBehavior(() => _currentTower.UpgradeTower(_currentTower.Data.Upgrades[0]));
                    _sellButton.UpdateState(_currentTower.Data.Upgrades[0].Price < _ressourceController.GoldCount);
                }
                else if (i == 2)
                {
                    _towerUpgradesButtons[2].gameObject.SetActive(true);
                    _towerUpgradesButtons[2].Initialize(_currentTower.Data.Upgrades[1].Icon, _currentTower.Data.Upgrades[1].Price);

                    _towerUpgradesButtons[2].ChangeBehavior(() => _currentTower.UpgradeTower(_currentTower.Data.Upgrades[1]));
                    _sellButton.UpdateState(_currentTower.Data.Upgrades[1].Price < _ressourceController.GoldCount);
                }
            }
        }
        ...
        else if (_currentTower.Data.Specs.Count > 0)
        {
            for (int i = 0; i < _towerUpgradesButtons.Count; i++)
            {
                ...
                _sellButton.UpdateState(_currentTower.Data.Specs[i].Price < _ressourceController.GoldCount);
            }
        }
